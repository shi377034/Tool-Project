...
{
    public class ActionFunction : FlowGraph
    {
        FlowGraph flowGraph
        {
            get
        ...
        private ActionFunctionInputNode _entry;
        private ActionFunctionOutputNode _exit;
        [SerializeField]
        ...
        public ActionFunctionInputNode entry
        {
            get
            {
                if (_entry == null)
                {
                    _entry = allNodes.OfType<ActionFunctionInputNode>().FirstOrDefault();
                    if (_entry == null)
                    {
                        _entry = AddNode<ActionFunctionInputNode>(new Vector2(-translation.x + 200, -translation.y + 200));
                    }
                }
                return _entry;
            }
        ...
        public ActionFunctionOutputNode exit
        {
            get
            {
                if (_exit == null)
                {
                    _exit = allNodes.OfType<ActionFunctionOutputNode>().FirstOrDefault();
                    if (_exit == null)
                    {
                        _exit = AddNode<ActionFunctionOutputNode>(new Vector2(-translation.x + 600, -translation.y + 200));
                    }
                }
                return _exit;
            }
        ...
        protected override void OnGraphValidate()
        {
            ...
            {
                var defIn = new DynamicPortDefinition("OnExecute", typeof(Flow));
                var defOut = new DynamicPortDefinition("EndAction", typeof(Flow));
        ...
        protected override GenericMenu OnCanvasContextMenu(GenericMenu menu, Vector2 mousePos)
        {
            ...
            foreach (DynamicPortDefinition port in inputDefinitions)
            {
                ...
                {
                    flowGraph.AddFlowNode<ActionFunctionInputPortNode>(mousePos, null, null).Register(port.ID);
                });
            }
            return menu;
        }
