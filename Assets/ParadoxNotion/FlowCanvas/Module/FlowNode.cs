	...
	abstract public class FlowNode : Node, ISerializationCallbackReceiver {
		...
		public Port GetOutputPort(string ID){
			...
			if (!outputPorts.TryGetValue(ID, out output)){
				// #if UNITY_EDITOR //update from previous version
				output = outputPorts.Values.FirstOrDefault(p => p.name.SplitCamelCase() == ID);
				// #endif
			}
			return output;
		}
		...
		void DeserializeInputPortValues(){
			...
			foreach (var pair in _inputPortValues){
				Port inputPort = null;
				if ( !inputPorts.TryGetValue(pair.Key, out inputPort) ){ //update from previous version
					// #if UNITY_EDITOR
					inputPort = inputPorts.Values.FirstOrDefault(p => p.name.SplitCamelCase() == pair.Key);
					// #endif
				}
		...
		///It is always recommended to use the generic versions.
		...
		///Reflection based registration if RegisterPorts is not overriden. Nowhere used by default.
		...
		///When overrided returns the base wild definition type with which new generic version can be made.
		...
		private static GUIStyle rightLabelStyle{
			get
			{
				if (_rightLabelStyle == null){
					...
					_rightLabelStyle.richText = true;
			...
			if (fullDrawPass || drawCanvas.Overlaps(nodeRect)){
				...
				if (orderedInputs != null){
					...
					for (var i = 0; i < orderedInputs.Length; i++){
						...
						port.pos = new Vector2(nodeRect.x - 5, nodeRect.y + port.posOffsetY);
				...
				if (orderedOutputs != null){
					...
					for (var i = 0; i < orderedOutputs.Length; i++){
						...
						port.pos = new Vector2(nodeRect.xMax + 5, nodeRect.y + port.posOffsetY);
						portRect.width = port.isConnected? 12:10;
						portRect.height = portRect.width;
						portRect.center = port.pos;
		...
		protected override void OnNodeGUI(){
			...
			if (orderedInputs != null && orderedInputs.Length > 0){
				for (var i = 0; i < orderedInputs.Length; i++){
					DrawNodePort(orderedInputs[i], leftLabelStyle);
				}
				GUILayout.Space(2);
			}
			...
			if (orderedOutputs != null && orderedOutputs.Length > 0){
				for (var i = 0; i < orderedOutputs.Length; i++){
					DrawNodePort(orderedOutputs[i], rightLabelStyle);
				}
				GUILayout.Space(2);
			}
			GUILayout.EndVertical();
		...
		void DrawNodePort(Port port, GUIStyle style){
			...
			if (portsLayoutHasDrawn == false && Event.current.type == EventType.Repaint){
				...
				port.pos = new Vector2(nodeRect.x, nodeRect.y + port.posOffsetY);
			}
		}
		...
		protected void DrawValueInputsGUI(){
			...
			foreach (var input in inputPorts.Values.OfType<ValueInput>() ){
				...
				input.serializedValue = EditorUtils.GenericField(input.displayName, input.serializedValue, input.type, null);
			}
		}
